...
(function() {
  ...
  var DOMCursor, prefix, ref, ref1,
    indexOf = [].indexOf;

  DOMCursor = (ref = window.DOMCursor) != null ? ref.DOMCursor : void 0;
  ...
  define([prefix + '/fingertree.js', prefix + '/prelude_ts.js', prefix + '/advice.js', prefix + '/lodash-4.17.2.min.js'], function(Fingertree, Prelude, Advice, _) {
    ...
    Observable = class Observable {
      ...
      trigger(type, ...args) {
        var j, len1, listener, ref2, results1;
        if (!this.suppressingTriggers) {
          ref2 = this.listeners[type] || [];
          results1 = [];
    ...
    FeatherJQ = class FeatherJQ extends Array {
      ...
      find(sel) {
        var j, len1, len2, node, o, ref2, ref3, result, results;
        results = $();
        ref2 = this;
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          node = ref2[j];
          if (node.querySelectorAll != null) {
            ref3 = node.querySelectorAll(sel);
      ...
      prop(name, value) {
        ...
        if (value != null) {
          ...
          return (ref3 = this[0]) != null ? ref3[name] : void 0;
        }
      }

      closest(sel) {
        var j, len1, n, node, ref2, result;
        result = $();
        ref2 = this;
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          node = ref2[j];
      ...
      data(key, value) {
        var j, len1, node, ref2, ref3, results1;
        if (!key) {
          return getUserData(this[0], true);
        } else if (value == null) {
          return (ref2 = getUserData(this[0], true)) != null ? ref2[key] : void 0;
        } else {
          ref3 = this;
          results1 = [];
      ...
      on(evtType, func) {
        var evt, j, len1, node, ref2, results1;
        ref2 = this;
        results1 = [];
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          node = ref2[j];
      ...
      off(evtType, func) {
        var events, h, j, len1, node, ref2, results1;
        ref2 = this;
        results1 = [];
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          node = ref2[j];
    ...
    runEvent = function(evt) {
      var handler, j, len1, ref2, ref3;
      ref3 = (ref2 = getEvents(evt.currentTarget)) != null ? ref2 : [];
      for (j = 0, len1 = ref3.length; j < len1; j++) {
        handler = ref3[j];
        handler(evt);
      }
    ...
    LeisureEditCore = class LeisureEditCore extends Observable {
      ...
      blockTextForNode(node) {
        var next, nextPos, parent, ref2;
        parent = this.blockNodeForNode(node)[0];
        if (next = (ref2 = this.options.getBlock(this.options.idForNode(node))) != null ? ref2.next : void 0) {
      ...
      moveCaretForVisibleNewlines(pos, e) {
        var dc;
        dc = typeof pos === 'number' ? this.domCursorForDocOffset(pos) : pos && indexOf.call(pos, 'type') >= 0 && indexOf.call(pos, 'node') >= 0 ? pos : this.domCursorForCaret(e);
      ...
      adjustCaretAfterMouseClick(e) {
        return requestAnimationFrame(() => {
          return this.moveCaretForVisibleNewlines(null, e.originalEvent);
        });
      }
      ...
      addKeyPress(e, c) {
        ...
        if (notShift = !shiftKey(c)) {
          ...
          for (i = j = 0, ref2 = Math.min(this.lastKeys.length, maxLastKeys); (0 <= ref2 ? j < ref2 : j > ref2); i = 0 <= ref2 ? ++j : --j) {
      ...
      moveDown() {
        var docPos, lastPos, line, linePos, lineTop, p, pos, prev, ref2;
        linePos = prev = pos = this.domCursorForCaret().save();
        if (!((ref2 = this.prevKeybinding) === keyFuncs.nextLine || ref2 === keyFuncs.previousLine)) {
      ...
      moveUp() {
        var docPos, lastPos, line, linePos, pos, prev, ref2;
        linePos = prev = pos = this.domCursorForCaret().save();
        if (!((ref2 = this.prevKeybinding) === keyFuncs.nextLine || ref2 === keyFuncs.previousLine)) {
          this.movementGoal = this.options.blockColumn(pos);
        }
      ...
      activateScripts(jq) {
        var activating, j, len1, newScript, ref2, results1, script, text;
        if (!activating) {
          activating = true;
          try {
            ref2 = jq.find('script');
    ...
    isAlphabetic = function(e) {
      var ref2;
      return !e.altKey && !e.ctrlKey && !e.metaKey && ((64 < (ref2 = eventChar(e)) && ref2 < 91));
    };
    BasicEditingOptions = (function() {
      ...
      class BasicEditingOptions extends Observable {
        ...
        mergeChangeContext(obj) {
          var ref2;
          return this.changeContext = _.merge({}, (ref2 = this.changeContext) != null ? ref2 : {}, obj);
        }
        ...
        validatePositions() {
          var block, cursor, j, node, offset, ref2;
          block = this.data.getBlock(this.data.getFirst());
          while (block) {
            if (node = this.nodeForId(block._id)[0]) {
              cursor = this.domCursor(node, 0).mutable();
              for (offset = j = 0, ref2 = block.text.length; (0 <= ref2 ? j < ref2 : j > ref2); offset = 0 <= ref2 ? ++j : --j) {
    ...
    computeNewStructure = function(access, oldBlocks, newText) {
      var newBlocks, next, offset, oldText, prev, ref2, ref3;
      prev = (ref2 = (ref3 = oldBlocks[0]) != null ? ref3.prev : void 0) != null ? ref2 : 0;
    ...
    DataStore = class DataStore extends Observable {
      ...
      load(name, text) {
        var block, blockMap, i, j, len1, newBlocks, prev, ref2;
        blockMap = {};
        ...
        this.first = (ref2 = newBlocks[0]) != null ? ref2._id : void 0;
      ...
      computeRemovesAndNewBlockIds(oldBlocks, newBlocks, newBlockMap, removes) {
        var i, j, len1, len2, newBlock, o, oldBlock, prev, ref2;
        ref2 = oldBlocks.slice(newBlocks.length, oldBlocks.length);
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          oldBlock = ref2[j];
      ...
      fingerNodeOrder(a, b) {
        var first, ref2, ref3, rest, split;
        return !(a || b) || (!a && b ? this.fingerNode(b) : !b && a ? this.fingerNode(a) : ([first, rest] = split = this.splitBlockIndexOnId(b), !first.isEmpty() && !rest.isEmpty() && ((ref2 = rest.peekFirst()) != null ? ref2.id : void 0) === b && ((ref3 = first.peekLast()) != null ? ref3.id : void 0) === a && split));
      }
      ...
      unindexBlock(id) {
        ...
        if (id) {
          ...
          if (((ref2 = rest.peekFirst()) != null ? ref2.id : void 0) === id) {
      ...
      blockForOffset(offset) {
        ...
        results = this.splitBlockIndexOnOffset(offset);
        return ((ref2 = (ref3 = results[1]) != null ? ref3.peekFirst() : void 0) != null ? ref2 : results[0].peekLast).id;
      }
      ...
      verifyIndex() {
        var bArray, blockIds, errs, iArray, j, last, len1, node, offset, ref2, treeIds;
        ...
        for (j = 0, len1 = iArray.length; j < len1; j++) {
          node = iArray[j];
          if (node.length !== ((ref2 = this.getBlock(node.id)) != null ? ref2.text.length : void 0)) {
    ...
    findEditor = function(node) {
      ...
      return (ref2 = target.data()) != null ? ref2.editor : void 0;
    };
